...
- **context_budget 语义**：Analyst 输入证据集的 token 上限；Researcher 按得分降序做**贪心装箱**（每块证据在剩余预算内才保留——低分小块可在高分大块装不下时幸存，得分密度优于严格排名截断），每次裁剪写一条 `context_trim` trace 事件（丢弃的 chunk_id + 原因），可观测、可回放。滚动摘要压缩不做（YAGNI）。主模型 500k 上下文，该预算是人为约束，正好构成成本-质量权衡叙事。
