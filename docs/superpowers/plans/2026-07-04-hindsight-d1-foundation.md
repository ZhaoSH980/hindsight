...
- **D2 review-carryover (from D1 task reviews)**: **[ALL ITEMS ①-⑪ COMPLETED in D2 — see plan 2026-07-05 Tasks 1-13; kept for the record.]** ① the orchestrator MUST wrap `registry.call` in a catch-all try/except returning `json.dumps({"error": str(exc)})` so the ReAct loop can self-correct — covers uncaught `OverflowError` (`9**9**9`), `RecursionError` (huge expressions), and `TypeError` (unexpected/missing kwargs from LLM JSON), plus a test for each; ② corpus_search excerpts should append a `"..."` truncation marker when text exceeds 700 chars; ③ chunker heading-only tiny chunks (a heading immediately followed by another heading becomes a 3-char chunk) — merge-forward polish if retrieval quality suffers; ④ D2 orchestrator wires sandbox `AuditEntry`s into trace as `TraceEvent(type="audit", payload=entry.model_dump(mode="json"))` (spec §5 amended) + a test for append-semantics across TraceRecorder instances on the same run_dir; ⑤ recorder open() should pass `newline="\n"` before demo runs/ get committed (avoids CRLF churn in git-tracked trace.jsonl); ⑥ D2 must coerce `temperature` (and any numeric param) to float before `chat()` — `chat(temperature=0)` vs `0.0` hash to different replay keys (route params through RunConfig or `float()` them); ⑦ D2 CostLedger wiring must extract usage keys explicitly (`usage.get("prompt_tokens", 0)`) — never `**usage` (xf-yun adds `question_tokens`; even standard `total_tokens` isn't in the signature); ⑧ add a CJK round-trip replay test when D2 touches recording.py (record Chinese content, reopen offline client, assert equal); ⑨ **D3**: RecordingLLMClient's sqlite connection is thread-bound — FastAPI threadpool needs `check_same_thread=False` + a lock (or per-call connections) and optionally `PRAGMA journal_mode=WAL`; ⑩ add a `tools`-list-order regression test for the replay key when D2 touches recording.py (two identical requests with reordered tools must produce two cache entries — protects tool-calling replay determinism against a future "helpful" sort); ⑪ **xf-yun rate-limits bursts**: probe hit `429 authorization failed` (code 11210) after ~4 rapid calls — D2 orchestrator's retry/backoff must handle 429 specifically with a long wait (10-30s) and pace sequential calls; recording layer makes retried runs cheap.
- **D3 carryover (current)**:
  - RecordingLLMClient sqlite is thread-bound (FastAPI needs `check_same_thread=False` + a lock, or per-request clients; `Store` is already thread-safe but coarse-locked).
  - UI must render failed runs (no `memo.md`) gracefully.
  - Leaderboard readers must use `.get("outcome")` (failed runs lack it).
  - Suite snapshot boundary is deliberately strict `<` (do not "fix" to `<=`).
  - Corpus citation-hygiene nits (`ai_capex_skeptic` URL, smci `bear_view` URL) → D4 polish.
- **D3 plan**: FastAPI routes + WebSocket per spec §5, React frontend (Studio / Trace / Eval pages), README skeleton.
- **D4 plan**: Leaderboard page, datasets 2-3, judge meta-eval labeling, GIF + bilingual README, offline rehearsal. Corpus polish carryover from the D1 data audit: `ai_capex_skeptic.md`'s cited URL is a 2025-03-26 news piece while the doc is dated 2025-05-10 — swap for a genuinely May-dated opinion source (citation hygiene). Same class of nit from the D2 audit: `smci_case3/bear_view_margins_governance.md`'s URL slug suggests Dec 2024 vs its 2025-02-24 date — swap for a Feb-2025 source.
